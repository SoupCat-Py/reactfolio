...
export default function ProjectCard({title, description, image, link, repo}: ProjectCardProps) {
    return (
        <article className="d-flex flex-col bg-bg border-primary radius-card">
            <div className="d-flex flex-col align-start justify-center rel">
                <h3 className={`color-primary m0 abs ${styles.title}`}>{title}</h3>
                <img className={styles.image} src={`/src/images/${image}.png`} alt={image}/>
            </div>
            <div className="d-flex flex-col align-start justify-start p2 g2">
                <span className={`body color-text ${styles.desc}`}>
                    {description}
                </span>
